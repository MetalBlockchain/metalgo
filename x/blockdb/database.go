...
func New(config DatabaseConfig, log logging.Logger) (_ database.HeightIndex, err error) {
	...
	db.locks, err = acquireDBLocks(config.IndexDir, config.DataDir)
	if err != nil {
		db.log.Error("Failed to acquire directory lock", zap.Error(err))
		return nil, err
	}
	defer func() {
		if err != nil {
			if rErr := db.locks.Release(); rErr != nil {
				db.log.Error("Failed to release directory lock after failed initialization", zap.Error(rErr))
			}
		}
	}()
...
func (db *Database) Close() error {
	...
	if err != nil {
		db.log.Error("Failed to persist index header", zap.Error(err))
	}

	db.closeFiles()

	if rErr := db.locks.Release(); rErr != nil {
		db.log.Error("Failed to release directory lock", zap.Error(rErr))
	}
...
func (db *Database) Put(height BlockHeight, block BlockData) error {
	db.closeMu.RLock()
...
func (db *Database) readBlockIndex(height BlockHeight) (indexEntry, error) {
...
func (db *Database) Get(height BlockHeight) (BlockData, error) {
	db.closeMu.RLock()
	defer db.closeMu.RUnlock()
...
func (db *Database) Has(height BlockHeight) (bool, error) {
	db.closeMu.RLock()
	defer db.closeMu.RUnlock()
	...
	return db.hasWithoutLock(height)
}

func (db *Database) hasWithoutLock(height BlockHeight) (bool, error) {
	_, err := db.readBlockIndex(height)
	if err != nil {
...
func (db *Database) getDataFileIndexForHeight(height BlockHeight) (int, error) {
	entry, err := db.readBlockIndex(height)
	if err != nil {
		return 0, err
	}
...
func (db *Database) readIndexEntry(height BlockHeight) (indexEntry, error) {
	var entry indexEntry
