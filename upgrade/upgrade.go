...
var (
	...
	Mainnet = Config{
		...
		// Granite (incl. ACP-181 L1 validator epochs) is deliberately NOT
		// scheduled on Metal networks in this merge. The activation timestamp
		// is a Metallicus decision: set it when a coordinated network upgrade
		// is planned. GraniteEpochDuration mirrors upstream and only takes
		// effect once GraniteTime is scheduled.
	...
	Tahoe = Config{
		...
		// Granite activation on Tahoe is a Metallicus decision — unscheduled
		// in this merge; see the Mainnet comment above.
		GraniteTime:          UnscheduledActivationTime,
		GraniteEpochDuration: 5 * time.Minute,
		HeliconTime:          UnscheduledActivationTime,
	}
