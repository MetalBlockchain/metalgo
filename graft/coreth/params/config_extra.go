...
func SetEthUpgrades(c *ChainConfig) error {
	...
	extra := GetExtra(c)
	// NOTE(metal): Metal's C-Chain (mainnet 381931 / Tahoe 381932) activated
	// Berlin and London via the Apricot upgrade schedule at genesis, so the
	// Avalanche mainnet/Fuji block-height special cases are removed here
	// (matches MetalBlockchain/coreth).
