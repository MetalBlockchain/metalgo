...
./bin/ginkgo -v ./tests/e2e -- --metalgo-path=./build/metalgo
```
...
without having to specify the `--metalgo-path` or `--plugin-dir`
flags.
...
```bash
./bin/ginkgo -v --label-filter=x ./tests/e2e -- --metalgo-path=./build/metalgo
```
...
```bash
./bin/gingko -v ./tests/e2e -- --metalgo-path=/path/to/metalgo --reuse-network
```
