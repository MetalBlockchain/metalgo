...
```bash
gh pr create --repo github.com/MetalBlockchain/metalgo --base master --title "chore: release $VERSION_RC"
```
...
cd $(mktemp -d)
go mod init test
go get github.com/MetalBlockchain/metalgo@"$VERSION_RC"
...
gh pr create --repo github.com/MetalBlockchain/metalgo --base master --title "chore: set require directives for $VERSION"
gh pr checks --watch
...
Create a release at [github.com/MetalBlockchain/metalgo/releases/new](https://github.com/MetalBlockchain/metalgo/releases/new):
...
1. Create PR and merge:
   ...
   gh pr create --repo github.com/MetalBlockchain/metalgo --base master --title "chore: prep next release $NEXT_VERSION"
   gh pr checks --watch
...
```bash
go test -run ^TestCompatibility$ github.com/MetalBlockchain/metalgo/graft/subnet-evm/plugin/evm
```
...
External consumers can then `go get github.com/MetalBlockchain/metalgo@v0.0.0-mybranch`.
